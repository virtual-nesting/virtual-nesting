...
**Virtual Nesting** — новый подход к разработке **файловой структуры** проектов. Облегчает работу с проектами на JavaScript.
...
1. [Решение — Virtual Nesting](#решение--virtual-nesting)
...
Любой проект — это коллекция программного кода. По мере развития проекта, растёт **объём** этого кода и **сложность** работы с ним.

То, как будет расти сложность, во многом зависит от принимаемых во время разработки решений.
...
Для того, чтобы упростить понимание, при работе с **кодом** придерживаются принятой в проекте **архитектуры** и **соглашений**, которые соотносятся с популярными шаблонами, практиками и индивидуальным опытом.
...
**Неудачно организованная** файловая структура **усложняет** это понимание. Более того, поскольку большинство файлов в проектах — модули, это также усложняет работу с **модулями**, понимание их **назначения** и **связей** между ними — одну из самых сложных задач при работе с проектами.

Из этого можно сделать вывод, что **неудачно организованная файловая структура — это проблема**.
...
- **Сложно понять принадлежность файла к определённой функции**

  Поскольку файлы сгруппированы только по типу, понять, какие из них относятся к определённой функции, чаще всего возможно только **через изучение кода**.

### Folders-by-feature

Подход предусматривает группировку файлов по принадлежности к функции.
...
Этот подход решает основную проблему подхода **folders-by-type** (отсутствие группировки по принадлежности к функции), но при большом количестве файлов тоже возникают **проблемы**:
...
- **Сложно ориентироваться**
  ...
  Поскольку в среднем или большом проекте группа может содержать десятки файлов и они **смешанного типа**, это **значительно** усложняет ориентирование. При просмотре директории мы знаем, что файлы относятся к определённой функции, но нам сложно понять, **какого они типа** и **сколько их**.
...
## Решение — Virtual Nesting

**Virtual Nesting** — новый подход к разработке **файловой структуры** проектов. Он вводит систему терминов ([категории](#категория), [группы](#группа), [элементы](#элемент)) и правил, чтобы облегчить работу с проектами на JavaScript.

Преимущества **Virtual Nesting**:
...
- группирует файлы по принадлежности к функции и типу одновременно 📦
...
Файловая структура проекта на React, разработанная с применением подхода **Virtual Nesting**, может выглядеть так:

- в `src` содержатся [**категории**](#категория), они группируют [**элементы**](#элемент) (файлы) по принадлежности к **функции**
...
- в **категориях** содержатся [**группы**](#группа), они группируют [**элементы**](#элемент) (файлы) по принадлежности к **типу**
...
> *Здесь мы создаём первые **категорию**, **группу** и **элемент**, знакомимся с их свойствами и ограничениями.*

Мы инициализировали новый репозиторий и собираемся добавить наши первые файлы. Начнём работу с создания корневого компонента и инициализации приложения.

Для начала создадим в `src` [категорию](#категория) `app`.

> **Категория** — это директория, которая применяется для группировки файлов по принадлежности к функции.

В категории `app` создадим [группу](#группа) `components`.

> **Группа** — это директория, которая применяется для группировки файлов по принадлежности к типу.
...
> **Важно!** **Virtual Nesting** определяет структуру вашего проекта, но не имена файлов и директорий. Все имена выбираете вы сами, исходя из популярных соглашений  и индивидуального опыта.
...
Совместно с менеджером мы решили, что из функций приложения стоит начать с реализации домашней страницы.
...
- Изучив макеты, мы понимаем, что на домашней странице используются общие для всего проекта элементы, такие как изображения профиля, кнопки, ссылки, всплывающие окна. Они входят в разработанный дизайнерами UI-кит, у нас есть его макеты и документация. 
  ...
  Попробуем начать с реализации **UI-кита**. Чтобы это не заняло много времени, реализуем минимальный набор компонентов и состояний, который нам необходим.
...
- На главной странице есть функции, которые впоследствии будут использоваться на других страницах - **посты**, **редактор постов** и **тренды**. Заведём для них отдельные категории.
...
> *Здесь мы знакомимся с разными вариантами реэкспорта ресурсов из категорий, примерами импорта в других категориях.*
...
> *Здесь мы создаём первую **подгруппу**, знакомимся с её свойствами и ограничениями.*
...
#### Усложняем `Post`, добавляем новую функцию — полноэкранный просмотр изображений

> *Здесь мы создаём первые **подкатегорию** и **мета-категорию**, знакомимся с их свойствами и ограничениями.*

Компонент отображения поста — это комплексная функция. Её можно разбить на более простые функции, например, полноэкранный просмотр изображений, прикреплённых к посту.

Функция полноэкранного просмотра является обособленной, но не может существовать в отрыве от основной функции. В этом случае для лучшей группировки стоит разделить их, но явно обозначить между ними связь.

Для этого создадим [подкатегорию](#подкатегории) `post.image-viewer`.

> Категории могут иметь подкатегории. Подкатегория — это директория, которая применяется для **декомпозиции комплексных функций**. 
> 
> Например, когда одну **комплексную** функцию будет удачнее разбить на несколько **простых**.
> 
...
В функции полноэкранного просмотра дублируются действия с постом - лайк, репост, реплай, поделиться.
...
Поздравляю, вы изучили **Virtual Nesting** на 90%. Структурируйте свои знания, изучив [методологию](#методология), и начните применять **Virtual Nesting** в своих проектах! ✌️

### Пример проекта

Посмотреть пример файловой структуры проекта можно на [CodeSandbox](https://codesandbox.io/s/virtual-nesting-react-v1-irwu2s).
...
Категория — директория, которая применяется для группировки [элементов](#элемент) по принадлежности к **функции**.
...
Например, когда для облегчения работы одну **комплексную** функцию будет лучше разбить на несколько **простых**.

Создаёт новое пространство имён.

Подкатегория отличается от обычной категории наличием **связи** с родительскими категориями (одной или несколькими) и связанным с этим **[ограничением](#ограничения-категорий)** на доступ к ресурсам.

Связь выражается через **виртуальную вложенность** — на уровне **имени** директории, через перечисление цепочки имён родительских категорий через точку. 
...
Группа — директория, которая применяется для группировки [элементов](#элемент) по принадлежности к **типу**. 
...
Подгруппа отличается от обычной группы наличием **связи** с родительскими группами (одной или несколькими) и связанным с этим **[ограничением](#ограничения-групп)** на доступ к ресурсам.

Связь выражается через **виртуальную вложенность** — на уровне **имени** директории, через разделение имени группы и имени подгруппы через хеш, затем через перечисление цепочки имён родительских подгрупп (если есть) через точку.

> ```
> # в примере ниже
>
> # `components#content` - подгруппа `components`
> # `components#content.text` - подгруппа `components#content`
>
> src
> |-- post
> |   |-- components
...
Модули внутри элемента-директории могут иметь связи.

Связь родительского модуля и потомка выражается через **виртуальную вложенность** — на уровне **имени** модуля, через *перечисление цепочки имён родительских модулей через точку*.
...
Архитектурные шаблоны и подходы к организации файловой структуры имеют общую цель — облегчить работу с проектами. 

Если архитектурный шаблон и подход к организации файловой структуры совместимы, их комбинированное применение может быть более эффективным в достижении этой цели.

Вы можете применить **Virtual Nesting** в вашем проекте, чтобы дополнительно облегчить работу, если архитектура проекта не включает требований к организации файловой структуры или эти требования совместимы с принципами **Virtual Nesting**.

Если требования архитектуры не позволяют вам применить **Virtual Nesting** полностью, вы также можете рассмотреть возможность применить отдельные идеи подхода. 
...
> В тексте часто встречается слово **функция**. Под функцией подразумевается совокупность свойств приложения, позволяющая выполнить определённую задачу.

## FAQ

**Можно ли использовать Virtual Nesting с TypeScript?**

Да!

**Можно ли использовать Virtual Nesting с CommonJS?**

Да! Только синтаксис модулей будет отличаться.

**Можно ли использовать Virtual Nesting с другими языками?**
